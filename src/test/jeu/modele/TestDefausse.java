...
class TestDefausse {
	...
	void testDefausse() {
		...
		assertNotNull(defausse.getDefausse(), "La défausse ne doit pas être " + "null.");
		assertTrue(defausse.getDefausse().isEmpty(), "La défausse doit " + "être vide à l'initialisation.");
	}
	...
	void testGetDefausse() {
		...
		assertNotNull(defausse.getDefausse(), "La pile de la défausse ne doit " + "pas être null.");
		assertTrue(defausse.getDefausse().isEmpty(), "La pile de la défausse " + "doit être vide.");

		// Vérifie que la défausse contient la même chose que la pile
		assertSame(pile, defausse.getDefausse(),
				"La méthode getDefausse() doit" + " renvoyer la même référence à la pile.");
	}
	...
	void testSetDefausse() {
		...
		assertEquals(2, defausse.getDefausse().size(), "La défausse doit " + "contenir 2 cartes.");

		// Avec une pile vide
		Stack<Carte> pileVide = new Stack<>();
		defausse.setDefausse(pileVide);
		assertEquals(pileVide, defausse.getDefausse(), "La défausse doit être " + "remplacée par une pile vide.");
		assertTrue(defausse.getDefausse().isEmpty(), "La défausse contenir 0 " + "carte.");
		...
		assertEquals(pileUnique, defausse.getDefausse(), "La défausse doit être" + " remplacée par la pile unique.");
		assertEquals(1, defausse.getDefausse().size(), "La défausse doit " + "contenir 1 carte.");
	}
