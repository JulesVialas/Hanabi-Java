...
class TestPioche {
	...
	void testPioche() {
		...
		assertEquals(50, pioche.getPioche().size(), "La pioche doit contenir 50" + " cartes.");
	...
	/**
	 * Méthode de test pour {@link jeu.modele.Pioche#setPioche(java.util.Stack)}.
	 */
	@Test
	void testSetPioche() {
		Pioche pioche = new Pioche();
		...
		assertEquals(nouvellePile, pioche.getPioche(), "La pioche doit " + "être remplacée par la nouvelle pile.");
		assertEquals(2, pioche.getPioche().size(), "La nouvelle pioche doit" + " contenir 2 cartes.");

		// Avec une pile vide
		Stack<Carte> pileVide = new Stack<>();
		pioche.setPioche(pileVide);

		assertEquals(pileVide, pioche.getPioche(), "La pioche doit être" + " remplacée par une pile vide.");
		assertEquals(0, pioche.getPioche().size(), "La pioche doit contenir " + "0 carte.");
		...
		assertEquals(pileCarteUnique, pioche.getPioche(),
				"La pioche doit " + "être remplacée par la pile avec une unique carte.");
		assertEquals(1, pioche.getPioche().size(), "La pioche doit contenir 1 " + "carte.");
	}
}
