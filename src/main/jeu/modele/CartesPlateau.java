...
/**
 * Les cinq piles de feux d'artifice posés sur le plateau de jeu. Une
 * pile de feux d'artifices peut être vide, ou comporter au maximum 5
 * cartes. Cette classe permet la manipulation de ces piles.
 *
...
public class CartesPlateau {
    ...
    /**
     * Instancie un objet de type CartesPlateau qui contient les piles de
     * cartes de chaque couleur du jeu Hanabi. Il n'est pas nécessaire de
     * définir la longueur maximale de la pile suite aux prérequis mis en
     * place pour ajouter une carte sur la pile.
     */
    ...
    /**
     * Pose une carte sur la pile de cartes rouges du plateau. La carte
     * doit être de couleur rouge, et sa valeur un cran au dessus de la
     * dernière carte posée.
     *
     * @param aPoser la carte a poser sur la pile de cartes rouges
     * @throws IllegalArgumentException si la couleur de la carte n'est
     *                                  pas ROUGE ou si la valeur n'est
     *                                  pas la suivante par rapport à la
     *                                  dernière carte de la pile
     */
    public void setPileRouge(Carte aPoser) {
        ...
        if (pileRouge.empty() && aPoser.getValeur() != Valeur.UN
                || (!pileRouge.empty()
                        && aPoser.getValeur().getValeurNumerique() != pileRouge
    ...
    /**
     * Pose une carte sur la pile de cartes jaunes du plateau. La carte
     * doit être de couleur jaune, et sa valeur un cran au dessus de la
     * dernière carte posée.
     *
     * @param aPoser la carte a poser sur la pile de cartes jaunes
     * @throws IllegalArgumentException si la couleur de la carte n'est
     *                                  pas JAUNE ou si la valeur n'est
     *                                  pas la suivante par rapport à la
     *                                  dernière carte de la pile
     */
    public void setPileJaune(Carte aPoser) {
        ...
        if (pileJaune.empty() && aPoser.getValeur() != Valeur.UN
                || (!pileJaune.empty()
                        && aPoser.getValeur().getValeurNumerique() != pileJaune
                                .peek().getValeur().getValeurNumerique() + 1)) {
            throw new IllegalArgumentException(ERREUR_MAUVAISE_VALEUR);
        }
    ...
    /**
     * Pose une carte sur la pile de cartes vertes du plateau. La carte
     * doit être de couleur verte, et sa valeur un cran au dessus de la
     * dernière carte posée.
     *
     * @param aPoser la carte a poser sur la pile de cartes vertes
     * @throws IllegalArgumentException si la couleur de la carte n'est
     *                                  pas VERT ou si la valeur n'est pas
     *                                  la suivante par rapport à la
     *                                  dernière carte de la pile
     */
    public void setPileVert(Carte aPoser) {
        ...
        if (pileVert.empty() && aPoser.getValeur() != Valeur.UN
                || (!pileVert.empty()
                        && aPoser.getValeur().getValeurNumerique() != pileVert
                                .peek().getValeur().getValeurNumerique() + 1)) {
            throw new IllegalArgumentException(ERREUR_MAUVAISE_VALEUR);
        }
    ...
    /**
     * Pose une carte sur la pile de cartes bleues du plateau. La carte
     * doit être de couleur bleue, et sa valeur un cran au dessus de la
     * dernière carte posée.
     *
     * @param aPoser la carte a poser sur la pile de cartes bleues
     * @throws IllegalArgumentException si la couleur de la carte n'est
     *                                  pas BLEU ou si la valeur n'est pas
     *                                  la suivante par rapport à la
     *                                  dernière carte de la pile
     */
    public void setPileBleu(Carte aPoser) {
        ...
        if (pileBleu.empty() && aPoser.getValeur() != Valeur.UN
                || (!pileBleu.empty()
                        && aPoser.getValeur().getValeurNumerique() != pileBleu
                                .peek().getValeur().getValeurNumerique() + 1)) {
            throw new IllegalArgumentException(ERREUR_MAUVAISE_VALEUR);
        }
    ...
    /**
     * Pose une carte sur la pile de cartes blanches du plateau. La carte
     * doit être de couleur blanche, et sa valeur un cran au dessus de la
     * dernière carte posée.
     *
     * @param aPoser la carte a poser sur la pile de cartes blanches
     * @throws IllegalArgumentException si la couleur de la carte n'est
     *                                  pas BLANC ou si la valeur n'est
     *                                  pas la suivante par rapport à la
     *                                  dernière carte de la pile
     */
    public void setPileBlanc(Carte aPoser) {
        ...
        if (pileBlanc.empty() && aPoser.getValeur() != Valeur.UN
                || (!pileBlanc.empty()
                        && aPoser.getValeur().getValeurNumerique() != pileBlanc
                                .peek().getValeur().getValeurNumerique() + 1)) {
            throw new IllegalArgumentException(ERREUR_MAUVAISE_VALEUR);
        }
