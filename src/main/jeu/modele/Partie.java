...
/**
 * Le jeu de société Hanabi est un jeu coopératif, c'est-à-dire un jeu dans
 * lequel les joueurs ne s'affrontent pas mais font équipe pour atteindre un
 * objectif commum. Ils interprètent ici des artificiers distraits qui ont
 * mélangé par mégarde les pourdres, mèches et fusées d'un grand feu d'artifice.
 * La représentation va commencer et c'est un peu la panique. Il leur faudra
 * s'entraider pour que le spectacle ne tourne pas à la catastrophe !
 *
 * Le but des artificiers est de composer 5 feux d'artifice, un de chaque
 * couleur (BLANC, ROUGE, BLEU, JAUNE, VERT), en constituant des suites
 * croissantes (1,2,3,4,5) avec des cartes de même couleur.
 *
...
public class Partie {
	...
	/**
	 * Une partie de Hanabi est caractérisée par un nombre de joueurs allant de 2 à
	 * 5, des paquets de cartes posés sur le plateau (vides au début de la partie)
	 * qui sont en fait les suites des feux d'artifices posés, des jetons bleus et
	 * rouges, une pioche et une défausse.
	 *
	 ...
	 */
	public Partie(String pseudoJ1, String pseudoJ2, String pseudoJ3, String pseudoJ4) {
		identifiant = PersistanceHanabi.genererIDPartie();
	 ...
	 */
	public Partie(String pseudoJ1, String pseudoJ2, String pseudoJ3, String pseudoJ4, String pseudoJ5) {
		identifiant = PersistanceHanabi.genererIDPartie();
	...
	/**
	 * Distribue le nombre de cartes souhaité de la pioche à un joueur. Cette
	 * méthode sert uniquement à la distribution lors du début de partie.
	 *
	...
	public int calculerScore() {
		...
		try {
			somme = somme + getFeuxPosesRouge().peek().getValeur().getValeurNumerique();
		} catch (EmptyStackException e) {
			// empty body; => si pile vide alors aucune carte du feu posée
		}

		try {
			somme = somme + getFeuxPosesJaune().peek().getValeur().getValeurNumerique();
		} catch (EmptyStackException e) {
			// empty body;
		}

		try {
			somme = somme + getFeuxPosesVert().peek().getValeur().getValeurNumerique();
		} catch (EmptyStackException e) {
			// empty body;
		}

		try {
			somme = somme + getFeuxPosesBleu().peek().getValeur().getValeurNumerique();
		} catch (EmptyStackException e) {
			// empty body;
		}

		try {
			somme = somme + getFeuxPosesBlanc().peek().getValeur().getValeurNumerique();
		} catch (EmptyStackException e) {
			// empty body;
		}
	...
	/**
	 * Indique si la partie est terminée. Pour que la partie soit terminée il faut
	 * que l'un des 3 cas suivant se présente:
	 * <ul>
	 * <li>les jetons rouges sont au nombre de 3</li>
	 * <li>tous les feux d'artifices sont complétés</li>
	 * <li>chaque joueur a joué une dernière fois après qu'un membre de l'équipe ait
	 * pioché la dernière carte (y compris ce dernier)</li>
	 * </ul>
	 *
