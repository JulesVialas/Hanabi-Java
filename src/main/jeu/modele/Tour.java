...
public class Tour {
	 ...
	 * bleus disponibles est strictement supérieur à zéro. Une carte peut recevoir
	 * plusieurs fois le même indice sans que cela pose problème. Lorsqu'un indice
	 * est donné, on le propage aux cartes de la main du joueur répondant au même
	 * critère.
	 *
	...
	public void donnerIndice(Carte recoitIndice, char natureIndice) {
		...
		for (Carte carte : recepteur.getCartesEnMains()) {
			if (natureIndice == 'c' && carte.getCouleurConnue() == recoitIndice.getCouleurConnue()) {
	...
	/**
	 * Propage l'indice donné à une carte aux autres cartes de la main du joueur
	 * répondant au même critères.
	 *
	 * @param natureIndice la nature de l'indice que l'on souhaite propager
	 * @param valeurIndice la carte dont on souhaite connaitre les caractéristiques
	 *                     pour pouvoir les propager
	 * @param artificier   le joueur qui possède la main dans laquelle on souhaite
	 *                     propager l'indice
	 * @throws IllegalStateException si la carte n'est pas dans la main d'un joueur.
	 */
	...
	/**
	 * Vérifie qu'une carte peut continue le feu d'artifice de sa couleur si elle
	 * est posée.
	 *
	 * @param aTester la carte dont on veut déterminer si elle continue ou non le
	 *                feux d'artifice de sa couleur.
	...
	private boolean doesContinueFirework(Carte aTester) {
		...
		case ROUGE:
			/*
			 * On teste si la valeur de la carte à poser est la suivante attendue, et si la
			 * valeur == 1 si la pile est vide pour éviter de throw EmptyStackException.
			 */
			return partieDuTour.getFeuxPosesRouge().empty() && aTester.getValeur() == Valeur.UN || aTester.getValeur()
					.getValeurNumerique() == partieDuTour.getFeuxPosesRouge().peek().getValeur().getValeurNumerique()
							+ 1;
		case JAUNE:
			return partieDuTour.getFeuxPosesJaune().empty() && aTester.getValeur() == Valeur.UN || aTester.getValeur()
					.getValeurNumerique() == partieDuTour.getFeuxPosesJaune().peek().getValeur().getValeurNumerique()
							+ 1;
		case VERT:
			return partieDuTour.getFeuxPosesVert().empty() && aTester.getValeur() == Valeur.UN || aTester.getValeur()
					.getValeurNumerique() == partieDuTour.getFeuxPosesVert().peek().getValeur().getValeurNumerique()
							+ 1;
		case BLEU:
			return partieDuTour.getFeuxPosesBleu().empty() && aTester.getValeur() == Valeur.UN || aTester.getValeur()
					.getValeurNumerique() == partieDuTour.getFeuxPosesBleu().peek().getValeur().getValeurNumerique()
							+ 1;
		default: // case Couleur.BLANC:
			return partieDuTour.getFeuxPosesBlanc().empty() && aTester.getValeur() == Valeur.UN || aTester.getValeur()
	...
	public void defausser(Carte carteADefausser) {
		...
		if (partieDuTour.getJetons().getBleus() < JetonsPlateau.NB_JETONS_BLEUS_MAX) {
