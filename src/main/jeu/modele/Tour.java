 ...
 * <ul>
 * <li>donner un indice sur la couleur d'une carte d'un de ses
 * équipiers;</li>
 * <li>donner un indice sur la valeur d'une carte d'un de ses
 * équipiers;</li>
...
public class Tour {
    ...
    private static final String ERREUR_JETONS_BLEUS_INSUFFISANTS = "Erreur: il n'y a pas assez de jetons bleus pour donner un indice";

    private static final String ERREUR_SAISIE_NATURE_INDICE = "Erreur: la nature de l'indice doit être 'c' pour couleur "
            + "ou 'v' pour valeur";

    private static final String ERREUR_INDICE_SOI_MEME = "Erreur: il n'est pas possible de donner un indice sur les cartes"
            + " de sa propre main";
    ...
    /**
     * Donne un indice sur une carte, couleur ou valeur, si le nombre de
     * jetons bleus disponibles est strictement supérieur à zéro. Une
     * carte peut recevoir plusieurs fois le même indice sans que cela
     * pose problème. Lorsqu'un indice est donné, on le propage aux cartes
     * de la main du joueur répondant au même critère.
     *
     * @param recepteur    Le joueur qui reçoit l'indice
     * @param recoitIndice la carte qui reçoit l'indice
     * @param natureIndice la nature de l'indice: 'c' pour la couleur, 'v'
     *                     pour la valeur
     * @throws IllegalArgumentException si natureIndice n'est ni 'c' ni
     *                                  'v'
     * @throws IllegalArgumentException si le joueur recevant l'indice
     *                                  n'est pas le joueur possédant la
     *                                  carte recevant l'indice
     * @throws IllegalArgumentException si la carte est null
     * @throws IllegalStateException    si le joueur recevant l'indice est
     *                                  le même qui donne l'indice
    ...
    public void donnerIndice(Carte recoitIndice, char natureIndice) {
        ...
        for (Carte carte : recepteur.getCartesEnMains()) {
            if (natureIndice == 'c' && carte.getCouleurConnue() == recoitIndice
                    .getCouleurConnue()) {
                carte.setCouleurConnue(true);
            } else if (natureIndice == 'v' && carte
                    .getValeurConnue() == recoitIndice.getValeurConnue()) {
    ...
    private Joueur determinerJoueurAyantCarte(Carte aSituer) {
        ...
        if (partieDuTour.getJoueur3().getCartesEnMains().contains(aSituer)) {

            return partieDuTour.getJoueur3();
        }

        if (partieDuTour.getJoueur4().getCartesEnMains().contains(aSituer)) {

            return partieDuTour.getJoueur4();
        }

        if (partieDuTour.getJoueur5().getCartesEnMains().contains(aSituer)) {
    ...
    /**
     * Propage l'indice donné à une carte aux autres cartes de la main du
     * joueur répondant au même critères.
     *
     ...
     * @param artificier   le joueur qui possède la main dans laquelle on
     *                     souhaite propager l'indice
    ...
    /**
     * Pose une carte sur les cartes posées sur le plateau si elle est
     * valide, sinon octroi un jeton rouge à l'équipe. Si la carte posée
     * complète un feu d'artifice, un jeton bleu est octroyé à l'équipe si
     * elle n'en possède pas déjà le nombre maximal. Si la pioche n'est
     * pas vide, une carte est ensuite ajoutée à la main du joueur.
     *
    ...
    public void poser(Carte carteAPoser) {
        ...
        if (doesContinueFirework(carteAPoser)) {
            ...
            carteAPoser.getValeur().getValeurNumerique() == 5
                    && partieDuTour.getJetons()
                            .getBleus() < JetonsPlateau.NB_JETONS_BLEUS_MAX) {
    ...
    /**
     * Vérifie qu'une carte peut continue le feu d'artifice de sa couleur
     * si elle est posée.
     *
     * @param aTester la carte dont on veut déterminer si elle continue ou
     *                non le feux d'artifice de sa couleur.
        ...
        try {
            ...
            case ROUGE:
                ...
                return partieDuTour.getFeuxPosesRouge().empty()
                        ...
                        || aTester.getValeur()
                                .getValeurNumerique() == partieDuTour
    ...
    /**
     * Défausse la carte du joueur et octroie un jeton bleu à l'équipe si
     * elle n'en possède pas déjà le nombre maximal. Si la pioche n'est
     * pas vide, une carte est ensuite ajoutée à la main du joueur.
     *
