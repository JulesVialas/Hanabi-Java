...
public class Tour {
	...
	public void donnerIndice(Joueur recepteur,Carte recoitIndice, char natureIndice) {
		...
		if (natureIndice != 'c' && natureIndice != 'v') {
			throw new IllegalArgumentException("Erreur: la nature de l'indice doit être 'c' pour couleur ou 'v' pour valeur");
		}
		boolean indiceDonne = false;
		for (Carte carte : recepteur.getCartesEnMains()) {
			if (natureIndice == 'c' 
					&& carte.getCouleurConnue() == recoitIndice.getCouleurConnue() 
					&& !indiceDonne) {
				carte.setCouleurConnue(true);
				indiceDonne = true;
			} else if (natureIndice == 'v' 
					    && carte.getValeurConnue() == recoitIndice.getValeurConnue() 
						&& !indiceDonne){
